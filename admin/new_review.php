...
<script>
	jQuery(function() {
		...
		jQuery('#categoryadd').click(function(e) {
			...
			if(categoryname.val()) {
				jQuery('#categories').append('<div class="list-group-item">' + categoryname.val() + '</div>');
				jQuery('#categories').append('<input type="hidden" name="ratingcategories[]" value ="' + categoryname.val() + '">');
				categoryname.val('');
			}
			e.preventDefault();
		});
		...
		<form action="" method="post" role="form" class="col-lg-6 col-md-12 col-xs-12">
			...
			<div class="form-group">
				...
				<div id="notifyemail" style="display: none; ">
					<h4><?php _e('Enter the email address to notify', 'star-review-manager'); ?></h4>
